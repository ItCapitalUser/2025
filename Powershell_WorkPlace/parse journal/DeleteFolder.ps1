...
$ListName = "DocumentsCopy"
$CSVPath = "C:\Temp\DocumentLibrary060924.csv"
$BatchSize = 500
...
$SiteURL= "https://smartholdingcom-my.sharepoint.com/personal/andrey_romaniv_ifsmart_com_ua"
$ListName = "Documents" #!!!
$CSVPath = "C:\Temp\DocumentLibrary30223_1.csv"
$BatchSize = 500
...
Try {
    ...
    $DataSort = $DataCollection  | Sort-Object Len -Descending
    $itemFind = $DataCollection   | Sort-Object Len -Descending Where-Object { $_.Name -like $fff } #Sort-Object Len -Descending #| Where-Object { $_.RelativeURL -like '*Documents/Документы/Проект А*' }
    $File = $Ctx.Web.GetFileByServerRelativeUrl($itemFind.RelativeURL)
    $Ctx.Load($File)
    $Ctx.ExecuteQuery()

    $itemFind.IdF

     [DateTime]::ParseExact($dateD, 'dd.MM.yy', $null)

     $dateD ="11.01.21"


$ListItem["Editor"] = $SPOUser

$ListItem["Modified"] = [DateTime]::ParseExact($dateD, 'dd.MM.yy', $null)
    $ListItem.Update()
    $Ctx.ExecuteQuery()


#Read more: https://www.sharepointdiary.com/2017/10/sharepoint-online-get-set-person-or-group-people-picker-field-using-powershell.html#ixzz8kxfBzzgX
 
    #Set metadata of the File
    $ListItem = $File.ListItemAllFields
    $ListItem["DateDoc"] =  [DateTime]::ParseExact($dateD, 'dd.MM.yy', $null)
 $ListItem.Update()
    $Ctx.ExecuteQuery()   


#Read more: https://www.sharepointdiary.com/2018/07/sharepoint-online-update-document-metadata-using-powershell.html#ixzz8kvafhoh4
